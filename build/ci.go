...
import (
	...
	"github.com/Daimon-Security/soros/crypto/signify"
	"github.com/Daimon-Security/soros/internal/build"
	"github.com/Daimon-Security/soros/params"
	"github.com/cespare/cp"
)

var (
	// Files that end up in the gsor*.zip archive.
	gethArchiveFiles = []string{
		"COPYING",
		executablePath("gsor"),
	}

	// Files that end up in the gsor-alltools*.zip archive.
	...
	debExecutables = []debExecutable{
		{
			BinaryName:  "abigen",
			Description: "Source code generator to convert Soros contract definitions into easy to use, compile-time type-safe Go packages.",
		},
		{
			BinaryName:  "bootnode",
			Description: "Soros bootnode.",
		},
		{
			BinaryName:  "evm",
			Description: "Developer utility version of the EVM (Soros Virtual Machine) that is capable of running bytecode snippets within a configurable environment and execution mode.",
		},
		{
			BinaryName:  "gsor",
			Description: "Soros CLI client.",
		},
		{
			BinaryName:  "puppeth",
			Description: "Soros private network manager.",
		},
		{
			BinaryName:  "rlpdump",
		...
		{
			BinaryName:  "clef",
			Description: "Soros account management tool.",
		},
	}
...
func doArchive(cmdline []string) {
	...
	var (
		...
		gsor     = "gsor-" + basegeth + ext
		alltools = "gsor-alltools-" + basegeth + ext
	)
	maybeSkipArchive(env)
	if err := build.WriteArchive(gsor, gethArchiveFiles); err != nil {
		log.Fatal(err)
	}
	if err := build.WriteArchive(alltools, allToolsArchiveFiles); err != nil {
		log.Fatal(err)
	}
	for _, archive := range []string{gsor, alltools} {
...
func archiveUpload(archive string, blobstore string, signer string, signifyVar string) error {
	...
	if signifyVar != "" {
		...
		untrustedComment := "verify with gsor-release.pub"
...
func doDocker(cmdline []string) {
	...
	//  - ethereum/client-go:latest                            - Pushes to the master branch, Gsoronly
	//  - ethereum/client-go:stable                            - Version tag publish on GitHub, Gsoronly
	//  - ethereum/client-go:alltools-latest                   - Pushes to the master branch, Gsor& tools
	//  - ethereum/client-go:alltools-stable                   - Version tag publish on GitHub, Gsor& tools
	//  - ethereum/client-go:release-<major>.<minor>           - Version tag publish on GitHub, Gsoronly
	//  - ethereum/client-go:alltools-release-<major>.<minor>  - Version tag publish on GitHub, Gsor& tools
	//  - ethereum/client-go:v<major>.<minor>.<patch>          - Version tag publish on GitHub, Gsoronly
	//  - ethereum/client-go:alltools-v<major>.<minor>.<patch> - Version tag publish on GitHub, Gsor& tools
	var tags []string
	...
	if len(*manifest) != 0 {
		...
		// Assemble and push the Gsormanifest image
		for _, tag := range tags {
...
func doDebianSource(cmdline []string) {
	var (
		...
		sshUser  = flag.String("sftp-user", "", `Username for SFTP upload (usually "gsor-ci")`)
...
func makeWorkdir(wdflag string) string {
	...
	if wdflag != "" {
		...
		wdflag, err = ioutil.TempDir("", "gsor-build-")
	}
	if err != nil {
		log.Fatal(err)
	}
...
func newDebMetadata(distro, goboot, author string, env build.Environment, t time.Time, name string, version string, exes []debExecutable) debMetadata {
	if author == "" {
		...
		author = "Soros Builds <fjl@ethereum.org>"
	}
	return debMetadata{
...
func doWindowsInstaller(cmdline []string) {
	...
	*workdir = makeWorkdir(*workdir)
	...
	for _, file := range allToolsArchiveFiles {
		...
		if filepath.Base(file) == "gsor.exe" {
			gethTool = file
		} else {
	...
	// first section contains the gsor binary, second section holds the dev tools.
	templateData := map[string]interface{}{
		"License":  "COPYING",
		"Geth":     gethTool,
		"DevTools": devTools,
	}
	build.Render("build/nsis.gsor.nsi", filepath.Join(*workdir, "gsor.nsi"), 0644, nil)
	...
	installer, _ := filepath.Abs("gsor-" + archiveBasename(*arch, params.ArchiveVersion(env.Commit)) + ".exe")
	build.MustRunCommand("makensis.exe",
		...
		"/DARCH="+*arch,
		filepath.Join(*workdir, "gsor.nsi"),
	)
...
func doAndroidArchive(cmdline []string) {
	...
	build.MustRun(gomobileTool("bind", "-ldflags", "-s -w", "--target", "android", "--javapkg", "org.ethereum", "-v", "github.com/Daimon-Security/soros/mobile"))

	if *local {
		// If we're building locally, copy bundle to build dir and skip Maven
		os.Rename("gsor.aar", filepath.Join(GOBIN, "gsor.aar"))
		os.Rename("gsor-sources.jar", filepath.Join(GOBIN, "gsor-sources.jar"))
		return
	}
	...
	archive := "gsor-" + archiveBasename("android", params.ArchiveVersion(env.Commit)) + ".aar"
	os.Rename("gsor.aar", archive)
...
func doXCodeFramework(cmdline []string) {
	...
	bind := gomobileTool("bind", "-ldflags", "-s -w", "--target", "ios", "-v", "github.com/Daimon-Security/soros/mobile")
	...
	// Create the archive.
	maybeSkipArchive(env)
	archive := "gsor-" + archiveBasename("ios", params.ArchiveVersion(env.Commit))
