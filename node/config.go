...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of gsor is "gsor". If no
...
func (c *Config) NodeName() string {
	...
	if name == "gsor" || name == "gsor-testnet" {
		name = "Geth"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "gsor" instances.
...
func (c *Config) ResolvePath(path string) string {
	...
	// by gsor 1.4 are used if they exist.
	if warn, isOld := isOldGethResource[path]; isOld {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn {
				c.warnOnce(&c.oldGethResourceWarning, "Using deprecated resource file %s, please move this file to the 'gsor' subdirectory of datadir.", oldpath)
			}
			return oldpath
		}
	}
