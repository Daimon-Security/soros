...
import (
	...
	"github.com/Daimon-Security/soros/cmd/utils"
	...
	"github.com/Daimon-Security/soros/ethclient"
...
var (
	...
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the Soros protocol")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for Soros and the faucet")
...
type request struct {
	...
	Account common.Address     `json:"account"` // Soros address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an Soros light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // Soros protocol stack
	client *ethclient.Client   // Client connection to the Soros chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*enode.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the Soros light client protocol
	...
	if err != nil {
		return nil, fmt.Errorf("Failed to register the Soros service: %w", err)
	}
...
// close terminates the Soros connection and tears down the faucet.
...
func (f *faucet) apiHandler(w http.ResponseWriter, r *http.Request) {
	...
	for {
		...
		// Retrieve the Soros address to fund, the requesting user and a profile picture
		var (
			id       string
			username string
			avatar   string
		...
		default:
			//lint:ignore ST1005 This error is to be displayed in the browser
			err = errors.New("Something funky happened, please open an issue at https://github.com/Daimon-Security/soros/issues")
		}
		if err != nil {
			if err = sendError(wsconn, err); err != nil {
...
// the uniqueness identifier (user id/username), username, avatar URL and Soros address to fund on success.
func authTwitter(url string, tokenV1, tokenV2 string) (string, string, string, common.Address, error) {
	...
	// and scrape it for the Soros address and profile URL. We need to load
	...
	if address == (common.Address{}) {
		//lint:ignore ST1005 This error is to be displayed in the browser
		return "", "", "", common.Address{}, errors.New("No Soros address found to fund")
	}
	var avatar string
...
// API, returning the user id, username, avatar URL and Soros address to fund on
// success.
...
// returning the username, avatar URL and Soros address to fund on success.
func authFacebook(url string) (string, string, common.Address, error) {
	...
	// scrape it for the Soros address and profile URL.
	//
...
// authNoAuth tries to interpret a faucet request as a plain Soros address,
