...
func TestAccountListEmpty(t *testing.T) {
	gsor := runGeth(t, "account", "list")
	gsor.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	gsor := runGeth(t, "account", "list", "--datadir", datadir)
	defer gsor.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	gsor := runGeth(t, "account", "new", "--lightkdf")
	defer gsor.ExpectExit()
	gsor.Expect(`
...
func importAccountWithExpect(t *testing.T, key string, expected string) {
	...
	gsor := runGeth(t, "account", "import", keyfile, "-password", passwordFile)
	defer gsor.ExpectExit()
	gsor.Expect(expected)
}
...
func TestWalletImport(t *testing.T) {
	gsor := runGeth(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer gsor.ExpectExit()
	gsor.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(gsor.Datadir, "keystore"))
	if len(files) != 1 {
...
func TestUnlockFlag(t *testing.T) {
	gsor := runMinimalGeth(t, "--port", "0", "--ipcdisable", "--datadir", tmpDatadirWithKeystore(t),
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(gsor.StderrText(), m) {
...
// https://github.com/Daimon-Security/soros/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	gsor.SetTemplateFunc("keypath", func(file string) string {
