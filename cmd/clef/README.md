...
This setup allows a DApp to connect to a remote Soros node and send transactions that are locally signed. This can help in situations when a DApp is connected to an untrusted remote Soros node, because a local one is not available, not synchronised with the chain, or is a node that has no built-in (or limited) account management.
...
GLOBAL OPTIONS:
   ...
   --keystore value        Directory for the keystore (default: "$HOME/.soros/keystore")
...
The general flow for signing a transaction using e.g. Gsoris as follows:
![image](sign_flow.png)

In this case, `gsor` would be started with `--signer http://localhost:8550` and would relay requests to `eth.sendTransaction`.
...
* Gsortodos
    ...
    - Relay: Gsorshould be started in `gsor --signer localhost:8550`.
    - Currently, the GsorAPIs use `common.Address` in the arguments to transaction submission (e.g `to` field). This type is 20 `bytes`, and is incapable of carrying checksum information. The signer uses `common.MixedcaseAddress`, which retains the original input.
    - The GsorAPI should switch to use the same type, and relay `to`-account verbatim to the external API.
* [x] Storage
  ...
  "params": [
    {
      "accounts": [
        {
          "url": "keystore:///home/bazonk/.soros/keystore/UTC--2017-11-20T14-44-54.089682944Z--123409812340981234098123409812deadbeef42",
          "address": "0x123409812340981234098123409812deadbeef42"
        },
        {
          "url": "keystore:///home/bazonk/.soros/keystore/UTC--2017-11-23T21-59-03.199240693Z--cafebabedeadbeef34098123409812deadbeef42",
