...
import (
	...
	"github.com/Daimon-Security/soros/accounts/keystore"
	"github.com/Daimon-Security/soros/common"
	"github.com/Daimon-Security/soros/common/fdlimit"
	...
	"github.com/Daimon-Security/soros/eth"
	...
	"github.com/Daimon-Security/soros/eth/tracers"
	"github.com/Daimon-Security/soros/ethdb"
	"github.com/Daimon-Security/soros/ethstats"
	"github.com/Daimon-Security/soros/graphql"
	"github.com/Daimon-Security/soros/internal/ethapi"
	"github.com/Daimon-Security/soros/internal/flags"
	"github.com/Daimon-Security/soros/les"
	"github.com/Daimon-Security/soros/log"
	"github.com/Daimon-Security/soros/metrics"
	"github.com/Daimon-Security/soros/metrics/exp"
	"github.com/Daimon-Security/soros/metrics/influxdb"
	...
	"github.com/Daimon-Security/soros/p2p/nat"
...
func MakeDataDir(ctx *cli.Context) string {
	if path := ctx.GlobalString(DataDirFlag.Name); path != "" {
		if ctx.GlobalBool(RopstenFlag.Name) {
			// Maintain compatibility with older Gsorconfigurations storing the
...
// for Gsorand returns half of the allowance to assign to the database.
...
func MakeAddress(ks *keystore.KeyStore, account string) (accounts.Account, error) {
	...
	log.Warn("Please use explicit addresses! (can search via `gsor account list`)")
	log.Warn("-------------------------------------------------------------------")
...
// RegisterEthService adds an Soros client to the stack.
// The second return value is the full node instance, which may be nil if the
// node is running as a light client.
func RegisterEthService(stack *node.Node, cfg *ethconfig.Config) (ethapi.Backend, *eth.Soros) {
	if cfg.SyncMode == downloader.LightSync {
		backend, err := les.New(stack, cfg)
		if err != nil {
			Fatalf("Failed to register the Soros service: %v", err)
		}
...
// RegisterEthStatsService configures the Soros Stats daemon and adds it to
// the given node.
func RegisterEthStatsService(stack *node.Node, backend ethapi.Backend, url string) {
	if err := ethstats.New(stack, backend, backend.Engine(), url); err != nil {
		Fatalf("Failed to register the Soros Stats service: %v", err)
	}
}
...
func SetupMetrics(ctx *cli.Context) {
	if metrics.Enabled {
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "gsor.", tagsMap)
		} else if enableExportV2 {
			tagsMap := SplitTagsFlag(ctx.GlobalString(MetricsInfluxDBTagsFlag.Name))

			log.Info("Enabling metrics export to InfluxDB (v2)")

			go influxdb.InfluxDBV2WithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, token, bucket, organization, "gsor.", tagsMap)
		}
...
// new format.
//
// e.g. gsor account new --keystore /tmp/mykeystore --lightkdf
//
// is equivalent after calling this method with:
//
// gsor --keystore /tmp/mykeystore --lightkdf account new
//
