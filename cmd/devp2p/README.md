...
Use `devp2p enrdump <base64>` to verify and display an Soros Node Record.
...
Run `devp2p key to-enode mynode.key -ip 127.0.0.1 -tcp 30305` to create an enode:// URL
...
1. initialize the gsor node with the `genesis.json` file contained in the `testdata` directory
2. import the `halfchain.rlp` file in the `testdata` directory
3. run gsor with the following flags:
```
gsor --datadir <datadir> --nodiscover --nat=none --networkid 19763 --verbosity 5
```

Then, run the following command, replacing `<enode>` with the enode of the gsor node:
 ```
...
To run the eth66 protocol test suite, initialize a gsor node as described above and run the following command,
replacing `<enode>` with the enode of the gsor node:
...
[dns-tutorial]: https://gsorsoroschain.org/docs/developers/dns-discovery-setup
