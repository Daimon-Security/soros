...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.soros", new NodeConfig());
			node.start();
...
func TestAndroid(t *testing.T) {
	...
	workspace, err := ioutil.TempDir("", "gsor-android-")
	if err != nil {
	...
	// Generate the mobile bindings for Gsorand add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.ethereum", "github.com/Daimon-Security/soros/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	cp.CopyFile(filepath.Join("libs", "gsor.aar"), "gsor.aar")
