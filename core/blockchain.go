...
// Package core implements the Soros consensus protocol.
package core
...
import (
	...
	"github.com/Daimon-Security/soros/common/prque"
	...
	"github.com/Daimon-Security/soros/internal/syncx"
...
// available in the database. It initialises the default Soros Validator and
// Processor.
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts, ancientLimit uint64) (int, error) {
	...
	writeAncient := func(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
		...
		//
		// But if the `TxIndexTail` is not nil, e.g. Gsoris initialized with
...
// sync, Gsorwill automatically construct the missing indices and delete
// the extra indices.
func (bc *BlockChain) maintainTxIndex(ancients uint64) {
	defer bc.wg.Done()

	// Before starting the actual maintenance, we need to handle a special case,
	// where user might init Gsorwith an external ancient database. If so, we
	...
	indexBlocks := func(tail *uint64, head uint64, done chan struct{}) {
		...
		// If the user just upgraded Gsorto a new version which supports transaction
