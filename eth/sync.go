...
func (h *handler) doSync(op *chainSyncOp) error {
	if op.mode == downloader.FastSync || op.mode == downloader.SnapSync {
		...
		// txlookup limit.
		// The main concern here is: during the fast sync Gsorwon't index the
		// block(generate tx indices) before the HEAD-limit. But if user changes
		// the limit in the next fast sync(e.g. user kill Gsormanually and
		// restart) then it will be hard for Gsorto figure out the oldest block
