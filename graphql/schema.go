...
const schema string = `
    ...
    scalar Bytes32
    # Address is a 20 byte Soros address, represented as 0x-prefixed hexadecimal.
    scalar Address
    ...
    # Account is an Soros account at a particular block.
    type Account {
    ...
    # Transaction is an Soros transaction.
    type Transaction {
    ...
    type Block {
        ...
        # Account fetches an Soros account at the current block's state.
    ...
    type Pending {
      ...
      transactions: [Transaction!]
      # Account fetches an Soros account for the pending state.
      account(address: Address!): Account!
    ...
    type Query {
        # Block fetches an Soros block by number or by hash. If neither is
