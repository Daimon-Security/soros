...
// Package gethclient provides an RPC client for gsor-specific APIs.
package gethclient
...
//
// If you want to use the standardized Soros RPC functionality, use ethclient.Client instead.
type Client struct {
	c *rpc.Client
}
...
// GCStats retrieves the current garbage collection stats from a gsor node.
...
// MemStats retrieves the current memory stats from a gsor node.
...
// GetNodeInfo retrieves the node info of a gsor node.
